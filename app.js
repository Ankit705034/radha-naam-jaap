...
$('#importInput').addEventListener('change', async (event) => { const [file] = event.target.files; if (!file) return; try { const backup = JSON.parse(await file.text()); const entries = backup && backup.entries; const target = Number(backup && backup.target); if (!entries || typeof entries !== 'object' || !Number.isFinite(target) || target < 1 || Object.values(entries).some((value) => !Number.isFinite(Number(value)) || Number(value) < 0)) throw new Error('invalid backup'); Object.keys(state.entries).forEach((key) => delete state.entries[key]); Object.entries(entries).forEach(([key, value]) => { state.entries[key] = Math.floor(Number(value)); }); state.target = Math.floor(target); save(); render(); showToast('Backup restore हो गया'); } catch (error) { showToast('Backup file सही नहीं है'); } event.target.value = ''; });
